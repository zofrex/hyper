...
impl Client {
    ...
    where U: AsRef<str> {
        ...
        self.tx.send(handler).unwrap();
        self.client.as_ref().unwrap().request(url.as_ref().parse().unwrap());
        rx
    }
}
...
fn client() -> Client {
    let (tx, rx) = mpsc::channel();
    let c = hyper::Client::new(move |_| rx.recv().unwrap()).unwrap();
    Client {
        client: Some(c),
        tx: tx,
    }
