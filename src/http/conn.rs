...
impl<T: Transport, H: MessageHandler<T>> Conn<T, H> {
    ...
    where F: MessageHandlerFactory<T, Output=H> {
        ...
        if events.is_error() {
            unimplemented!("error event unimplemented");
        }
        ...
        let new_state = match (state, next.interest) {
            ...
            (State::Http1(http1), Next_::End) => {
                match (http1.reading, http1.writing) {
                    ...
                    (reading, Writing::Ready(mut encoder)) => {
                        if let Some(trailer) = encoder.trailer() {
                            let h1 = Http1 {
                                reading: reading,
                                writing: Writing::Chunk(Chunk {
                                    buf: Cow::Borrowed(trailer),
                                    pos: 0,
                                    next: (encoder, Next::end())
                                }),
                                ..http1
                            };
                            State::Http1(h1)
